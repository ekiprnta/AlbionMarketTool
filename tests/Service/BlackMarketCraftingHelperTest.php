...
class BlackMarketCraftingHelperTest extends TestCase
{
    ...
    public function testCalculateResource(string $entityName, string $entityTier, string $name, string $tier): void
    {
        ...
        $expectedResourceEntity->getTier()->willReturn($entityTier);
    ...
    public function testCalculateJournalNull(
        ...
        $expectedJournalEntity->getTier()->willReturn($entityTier);
        $expectedJournalEntity->getFillStatus()->willReturn($entityFillStatus);
    ...
    {
        return [
            [6.3, 32, 0.3, 0.025, 2000, 9],
            [17.3, 8, 0.8, 0.25, 2000, 14],
            [76, 988, 4, 23, 400032, 5],
        ];
    }
    ...
    public function testCalculateProfitJournals(
        ...
        $this->assertEqualsWithDelta(
            $expectedResult,
            $this->bmcHelper->calculateProfitJournals(
                $emptyJournalPrice,
                $fullJournalPrice,
                $journalAmount,
            ),
    ...
    {
        return [
            [9876, 62398, 40, 1938645.2],
            [98765, 34567, 7, -465113.985],
            [157650, 2344, 365, -56742301.4],
        ];
    }

    /** @dataProvider getItemValue */
    public function testCalculateItemValue(
        int $totalAmount,
        int $price,
        float $expectedResult
    ): void {
        $this->assertEqualsWithDelta(
            $expectedResult,
            $this->bmcHelper->calculateItemValue(
                $totalAmount,
                $price,
            ),
            0.00000001
    ...
    {
        return [
            [42000, 16, 44300, 8, 1000740.0],
            [1343, 12, 65, 20, 16980.6],
            [2345, 8, 3456, 6, 38508.6],
        ];
    }
    ...
    {
        return [
            [42000, 16, 44300, 8, 1026400.0],
            [1343, 12, 65, 20, 17416.0],
            [2345, 8, 3456, 6, 39496.0],
        ];
    }
}