...
class BlackMarketTransportingHelperTest extends TestCase
{
    ...
    public function testCalculateCityItemException(
        ...
        $this->cityItem->getTier()->willReturn($cityTier);
        $this->cityItem->getName()->willReturn($cityItem);
        $this->bmItem->getTier()->willReturn($bmTier);
        $this->bmItem->getName()->willReturn($bmItem);
    ...
    {
        $this->cityItem->getTier()->willReturn('a');
        $this->cityItem->getName()->willReturn('b');
        $this->bmItem->getTier()->willReturn('a');
        $this->bmItem->getName()->willReturn('b');

        $this->assertEquals($this->cityItem->reveal(), $this->bmtHelper->calculateCityItem($this->bmItem->reveal(), [$this->cityItem->reveal()]));
    }
    ...
    public function testCalculateSingleProfit(int $bmPrice, int $cityPrice, float $expectedResult): void {
        ...
        $this->assertEqualsWithDelta($expectedResult, $this->bmtHelper->calculateSingleProfit($bmPrice, $cityPrice), 0.0000001);
    }
    ...
    public function testCalculateProfit(float $singleProfit, int $amount, float $expectedResult): void {
        ...
        $this->assertEqualsWithDelta($expectedResult, $this->bmtHelper->calculateProfit($singleProfit, $amount), 0.0000001);
    }
    ...
    public function testCalculateAmount(int $weight, float $itemWeight, float $expectedResult): void {
        ...
        $this->assertEqualsWithDelta($expectedResult, $this->bmtHelper->calculateAmount($weight, $itemWeight), 0.0000001);
    }
